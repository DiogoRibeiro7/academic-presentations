"""Validate canonical author metadata against the shared Beamer helper theme."""
...
def validate() -> None:
    """Fail if the helper theme drifts from canonical author metadata."""
    metadata = _load_author_metadata()
    theme = THEME_FILE.read_text(encoding="utf-8")

    parts = metadata["name"].split(",", maxsplit=1)
    if len(parts) != 2:
    ...
    if missing:
        raise ValueError(
            "shared Beamer helper theme is out of sync with shared/author.json: "
            + "; ".join(missing)
        )
    ...
    if stale:
        raise ValueError(
            "shared Beamer helper theme still contains stale metadata: "
            + "; ".join(stale)
        )
