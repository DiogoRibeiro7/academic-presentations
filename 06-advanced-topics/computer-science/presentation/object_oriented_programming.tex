...
% Pseudocode language definition. Its visual style inherits the canonical model.
...
\begin{frame}{Inheritance and Subtyping \mins{5}}
  \begin{itemize}
    \item Inheritance is a language mechanism for sharing or specializing implementation; subtyping is a behavioral relationship about substitutability.
    \item The two often coincide in class-based languages but are conceptually distinct.
    \item Reuse alone is weak justification for inheritance when the subtype cannot honor the supertype contract.
  \end{itemize}
\end{frame}
...
\begin{frame}{SOLID as Design Heuristics \mins{10}}
  \begin{itemize}
    \item \textbf{Single Responsibility}: seek cohesive responsibilities and understandable reasons for change; the boundary is a design judgment, not a measurable law.
    \item \textbf{Open/Closed}: prefer extension mechanisms when variation is expected, but avoid speculative abstractions for changes that may never occur.
    \item \textbf{Liskov Substitution}: subtypes should preserve the behavioral expectations clients rely on, including invariants and meaningful pre/postconditions.
    \item \textbf{Interface Segregation}: avoid forcing clients to depend on operations they do not need; interface size should follow actual collaboration boundaries.
    \item \textbf{Dependency Inversion}: place stable policy behind abstractions when that reduces coupling; an interface around every dependency can create noise without flexibility.
  \end{itemize}

  \begin{alertbox}{Use principles diagnostically}
    SOLID helps reason about coupling and change. It does not replace domain modeling, performance analysis, testing, or empirical evidence about maintainability.
  \end{alertbox}
\end{frame}

\begin{frame}{Composition and Inheritance \mins{5}}
  \begin{itemize}
    \item Composition can localize variation and avoid coupling to a superclass implementation.
    \item Inheritance can be appropriate when the subtype relation is behaviorally meaningful and the inherited contract is stable.
    \item Choose between them by substitutability, lifecycle, extension pressure, testing cost, and language/runtime constraints rather than by a blanket rule.
  \end{itemize}
\end{frame}

\begin{frame}{Law of Demeter as a Coupling Heuristic \mins{3}}
  \begin{itemize}
    \item Long navigation chains can reveal that a client knows too much about another object's internal structure.
    \item The heuristic encourages narrow collaboration boundaries, but fluent APIs, immutable value objects, and query models can legitimately use chained calls.
    \item Evaluate the dependency created by the chain rather than counting dots mechanically.
  \end{itemize}
\end{frame}
...
\begin{frame}{Common Pitfalls \mins{6}}
  \begin{itemize}
    ...
    \item Anemic domain models when the domain genuinely requires behavior-rich invariants; simple data-transfer objects are not automatically a design smell.
...
\begin{frame}{Heuristics \mins{4}}
  \begin{itemize}
    ...
    \item Prefer cohesive responsibilities with a comprehensible change boundary.
...
\begin{frame}{Synthesis}
  \begin{itemize}
    \item Object orientation is one way to organize state, behavior, identity, and collaboration. It is most useful when those concepts map cleanly to the problem and lifecycle of the software.
    \item Encapsulation is about preserving invariants and controlling dependencies, not simply making fields private.
    \item Subtyping is behavioral: a subtype is useful only when clients can rely on the same contract without special-case knowledge.
    \item SOLID, composition, Law of Demeter, and design patterns are heuristics for managing change and coupling. They should be tested against actual maintenance, performance, and complexity costs.
    \item Procedural, functional, data-oriented, and object-oriented designs can coexist in the same system; architecture should follow the dominant problem structure rather than ideology.
  \end{itemize}
\end{frame}
...
\begin{frame}{Selected References}
  \small
  \begin{itemize}
    \item B. Meyer (1997). \textit{Object-Oriented Software Construction}, 2nd ed. Prentice Hall.
    \item B. Liskov and J. M. Wing (1994). "A Behavioral Notion of Subtyping." \textit{ACM Transactions on Programming Languages and Systems}, 16(6), 1811--1841.
    \item E. Gamma, R. Helm, R. Johnson, and J. Vlissides (1994). \textit{Design Patterns}. Addison-Wesley.
    \item M. Fowler (2018). \textit{Refactoring}, 2nd ed. Addison-Wesley.
  \end{itemize}
\end{frame}
