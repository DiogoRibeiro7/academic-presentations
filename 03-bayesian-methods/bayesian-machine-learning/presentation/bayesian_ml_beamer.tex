...
\begin{frame}{Variational Bayesian Neural Networks}
A common BNN approximation places a tractable variational distribution over weights:
\[
  q_\phi(w) \approx p(w\mid\mathcal D).
\]

\begin{itemize}
  \item location parameters $\mu$ represent posterior means in the approximation;
  \item scale parameters are constrained positive, often through a softplus transform;
  \item the reparameterization trick writes $w=\mu+\sigma\epsilon$ with $\epsilon\sim\mathcal N(0,1)$ so gradients can propagate through sampling.
\end{itemize}

\begin{alertblock}{Approximation}
Sampling weights from a variational layer is only one component. Training must optimize an ELBO or another justified objective that includes both data fit and divergence to the prior.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Bayesian Linear Layer: Core Mechanics}
...
class BayesianLinear(nn.Module):
    ...
    def sample_weight(self):
        ...
        return self.weight_mu + sigma * eps
\end{lstlisting}

\begin{block}{Prediction}
Posterior-predictive summaries use multiple weight samples or another approximation to integrate over uncertainty in $q_\phi(w)$.
\end{block}
\end{frame}
