...
\begin{frame}{Randomized Experiments: Identification by Assignment}
Under random assignment, treatment is independent of potential outcomes by design:
\[
  T \perp \{Y(1),Y(0)\}.
\]

\begin{itemize}
  \item This identifies the intention-to-treat effect of assignment when outcomes are observed according to the randomized design.
  \item Covariate balance holds in expectation; realized samples can still be imbalanced by chance.
  \item Analysis should respect the randomization unit, clustering, blocking, and any pre-specified design features.
\end{itemize}
\end{frame}

\begin{frame}{Randomized Experiments: Practical Limits}
\begin{itemize}
  \item \textbf{Non-compliance}: assignment and received treatment may differ; ITT and treatment-on-the-treated estimands are different.
  \item \textbf{Attrition}: missing outcomes can break the simple randomized comparison if loss to follow-up is informative.
  \item \textbf{Interference}: one unit's treatment can affect another unit's outcome.
  \item \textbf{External validity}: an internally valid experiment need not generalize to other populations, settings, or implementations.
  \item \textbf{Ethics/feasibility}: some interventions cannot be randomized.
\end{itemize}

\begin{alertblock}{What randomization buys}
Randomization addresses confounding in treatment assignment for the randomized estimand. It does not automatically solve non-compliance, missing data, interference, measurement error, or generalizability.
\end{alertblock}
\end{frame}

\begin{frame}{Instrumental Variables: Identification}
An instrument $Z$ can identify variation in treatment $X$ that is not driven by the unobserved causes of outcome $Y$.

\begin{enumerate}
  \item \textbf{Relevance}: $Z$ changes the distribution of $X$.
  \item \textbf{Independence}: $Z$ is independent of the potential outcomes/treatment types under the design.
  \item \textbf{Exclusion}: $Z$ affects $Y$ only through its effect on $X$.
\end{enumerate}

\begin{alertblock}{Interpretation}
Instrument validity is primarily a design/substantive argument. A strong first stage is observable; independence and exclusion are not generally established by the outcome data alone.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Instrumental Variables: Two-Stage Least Squares}
For a linear constant-effect model with one instrument, 2SLS uses instrument-induced treatment variation:
...
\begin{block}{Why use an IV estimator directly?}
Regressing $Y$ on fitted $\hat X$ with ordinary OLS can recover the point estimate in simple cases, but naive second-stage standard errors are wrong because $\hat X$ is estimated. Dedicated IV routines handle the covariance calculation and diagnostics.
\end{block}
\end{frame}

\begin{frame}{Instrumental Variables: Diagnostics and Scope}
\begin{columns}
\begin{column}{0.48\textwidth}
\textbf{Diagnostics}
\begin{itemize}
  \item inspect first-stage strength;
  \item report weak-instrument-robust inference where needed;
  \item assess sensitivity to alternative instrument sets/specifications;
  \item justify exclusion and independence substantively.
\end{itemize}
\end{column}
...
\begin{itemize}
  ...
  \item IV estimands can differ from population-average effects;
  \item with binary treatment/instrument, monotonicity plus standard IV assumptions yields a LATE for compliers.
\end{itemize}
\end{column}
\end{columns}

\begin{alertblock}{LATE}
The complier interpretation is not automatic for every IV model; it relies on the binary-instrument potential-outcomes framework and assumptions such as monotonicity.
\end{alertblock}
\end{frame}
...
\begin{frame}{Common Causal-Inference Failure Modes}
\begin{itemize}
  \item \textbf{Association interpreted causally}: regression coefficients are given causal meaning without an identification argument.
  \item \textbf{Bad adjustment}: important common causes are omitted, mediators are controlled when targeting total effects, or colliders are conditioned on.
  \item \textbf{Weak or invalid instruments}: first-stage weakness, exclusion violations, or incorrect interpretation of the IV estimand.
  \item \textbf{Design assumptions ignored}: non-parallel trends, manipulation near an RD cutoff, or inadequate overlap/hidden confounding in observational adjustment.
\end{itemize}

\begin{alertblock}{Core lesson}
A sophisticated estimator cannot repair a design that does not identify the target causal effect.
\end{alertblock}
\end{frame}

\begin{frame}{Credibility Checklist for Causal Claims}
\begin{itemize}
  \item state the estimand and causal assumptions explicitly;
  \item use DAGs/design knowledge to justify adjustment or assignment mechanisms;
  \item inspect diagnostics relevant to the design, while recognizing they cannot prove untestable assumptions;
  \item report sensitivity analyses and alternative specifications where meaningful;
  \item distinguish statistical uncertainty from identification uncertainty;
  \item discuss external validity and practical significance separately from internal identification.
\end{itemize}

\begin{block}{Triangulation}
Agreement across designs with different identifying assumptions can strengthen an argument, but multiple biased estimators do not become causal merely because they agree.
\end{block}
\end{frame}
