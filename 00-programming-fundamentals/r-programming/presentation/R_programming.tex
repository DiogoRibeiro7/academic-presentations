  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      \begin{itemize}
        ...
        \item \textbf{Multi-paradigm}: vectorized, functional, imperative, and object-oriented styles
    ...
    \begin{column}{0.4\textwidth}
      ...
      \vspace{1em}
      \small Current release: R 4.6.1
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{R in the Data-Science Ecosystem}
  \begin{itemize}
    \item R is particularly strong in statistical modeling, visualization, reproducible analysis, and domain-specific scientific packages.
    \item Python offers a broader general-purpose software ecosystem and is often preferred when analysis must integrate tightly with application code, services, or deep-learning infrastructure.
    \item SAS, SPSS, Julia, SQL, and other tools remain appropriate in specific institutional, regulatory, numerical, or operational settings.
    \item Tool choice should follow the analytical requirements, deployment environment, team expertise, reproducibility needs, and available libraries rather than a universal ranking.
  \end{itemize}

  \begin{block}{R is multi-paradigm}
    R supports vectorized functional programming, first-class functions, object-oriented systems (S3/S4/R6), and imperative control flow. It is not usefully described as belonging to only one programming paradigm.
  \end{block}
\end{frame}
...
\begin{frame}[fragile]{Getting Started: Installation}
  \begin{enumerate}
    ...
    \item \textbf{Install an IDE if desired}: Posit RStudio, VS Code with an R extension, or another editor/notebook workflow
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Pipe operator}: Chain operations with \texttt{\%>\%}
...
\begin{frame}[fragile]{Quarto / R Markdown for Reproducible Reports}
\begin{lstlisting}
# YAML header example
---
...
\begin{lstlisting}
# Vectorized operation vs explicit loop
# Often faster and clearer for elementwise arithmetic
result <- numeric(1000)
for (i in 1:1000) {
  result[i] <- i^2
}
...
# Pre-allocate memory
# Avoid repeatedly growing objects
x <- numeric(0)
for (i in 1:1000) {
  x <- c(x, i^2)  # repeated allocation/copying can be expensive
}

# Prefer pre-allocation when an explicit loop is appropriate
x <- numeric(1000)
for (i in 1:1000) {
  x[i] <- i^2
}

# Use the clearest primitive for the task; apply is not inherently faster than a loop
data <- matrix(rnorm(10000), nrow = 100)
row_means <- rowMeans(data)  # specialized vectorized primitive
col_means <- colMeans(data)

# Explicit loops can also be efficient when objects are pre-allocated
\end{lstlisting}
\end{frame}
...
\begin{frame}{Final Exercise: Complete Project (30 min)}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Analysis}:
      ...
      \end{itemize}
    \item \textbf{Report}: Create a Quarto or R Markdown document with:
      \begin{itemize}
...
\begin{frame}{Advanced R Topics to Explore}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item Machine learning with \texttt{tidymodels}, \texttt{mlr3}, or domain-specific packages
        \item Time series analysis with \texttt{fable}, \texttt{forecast}, and related packages
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item APIs with \texttt{httr2}, \texttt{curl}, \texttt{jsonlite}
...
\begin{frame}{Learning Resources}
  ...
  \begin{itemize}
    \item Posit learning resources: \url{https://posit.co/resources/}
  ...
  \textbf{Communities:}
  \begin{itemize}
    \item R community spaces, including \texttt{\#rstats}, Posit Community, and local user groups
...
\begin{frame}{Synthesis}
  \begin{itemize}
    \item R is a multi-paradigm language designed around vectorized data analysis, statistical computing, graphics, and interactive exploration.
    \item Vectorization is useful when it maps naturally to the problem, but explicit loops are not inherently wrong. Pre-allocation, appropriate primitives, algorithmic complexity, and profiling matter more than style slogans.
    \item Reproducibility requires more than a script: record package dependencies, inputs, random seeds, computational environment, and the commands needed to rebuild the analysis.
    \item Base R, tidyverse, data.table, tidymodels, Bioconductor, and other ecosystems solve different problems. Consistency within a project matters more than allegiance to one style.
    \item For serious analytical work, correctness and clarity should be established before micro-optimization; profile first, then optimize the actual bottleneck.
  \end{itemize}
\end{frame}
...
\begin{frame}{Selected References}
  \small
  \begin{itemize}
    \item R Core Team. \textit{R: A Language and Environment for Statistical Computing}. R Foundation for Statistical Computing.
    \item H. Wickham, M. Çetinkaya-Rundel, and G. Grolemund (2023). \textit{R for Data Science}, 2nd ed. O'Reilly.
    \item H. Wickham (2019). \textit{Advanced R}, 2nd ed. Chapman \& Hall/CRC.
    \item H. Wickham and J. Bryan (2023). \textit{R Packages}, 2nd ed. O'Reilly.
    \item G. Grolemund and H. Wickham (2014). "A Cognitive Interpretation of Data Analysis." \textit{International Statistical Review}, 82(2), 184--204.
  \end{itemize}
\end{frame}
