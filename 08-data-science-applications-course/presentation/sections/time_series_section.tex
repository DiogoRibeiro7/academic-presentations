...
\begin{column}{0.5\textwidth}
\textbf{Method choice depends on the forecasting problem:}
...
\hline
\textbf{ARIMA} & Linear dependence after suitable differencing/transformation \\
\hline
\textbf{Exp Smoothing} & Trends + seasonality, simple \\
\hline
\textbf{Additive trend/seasonal models} & Structured trend, seasonality, calendar effects \\
\hline
\textbf{XGBoost} & Many features, non-linear \\
\hline
\textbf{RNN/LSTM} & Sequential nonlinear models when data volume and covariates justify them \\
\hline
\textbf{Transformer-based} & Multi-series/covariate settings where the architecture earns its added complexity \\
\hline
\end{tabular}
\end{table}
