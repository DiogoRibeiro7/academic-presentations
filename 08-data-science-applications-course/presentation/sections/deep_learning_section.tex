...
Under standard universal-approximation conditions, a sufficiently wide network with an appropriate non-linear activation can approximate continuous functions on compact sets arbitrarily well; the theorem does not imply efficient learning, finite-sample generalization, or practical trainability.
\end{column}
...
\begin{frame}[fragile,allowframebreaks]{Optimization Algorithms}
...
\begin{lstlisting}[language=python,caption={PyTorch optimizer + scheduler}]
...
\item \textcolor{crimson}{\textbf{Batch Normalization:}}
   ...
   \item Can have regularizing effects in some training regimes, but its main role is optimization/normalization of intermediate activations
   \end{itemize}
