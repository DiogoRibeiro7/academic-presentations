...
\begin{frame}{Containerization: What It Solves}
\begin{itemize}
  \item package application code and runtime dependencies into a repeatable image;
  \item reduce environment drift between development, CI, and deployment;
  \item isolate services and make deployment units easier to test and roll back;
  \item provide a standard artifact for orchestration platforms.
\end{itemize}

\begin{alertblock}{Limit}
A container improves environment reproducibility, but it does not guarantee identical external data, hardware behavior, model artifacts, secrets, or service dependencies.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Containerization: Minimal Dockerfile}
...
\begin{block}{Build discipline}
Pin dependencies, keep images small, run as a non-root user where possible, scan images, and separate configuration/secrets from the image.
\end{block}
\end{frame}
...
\begin{frame}{ML CI/CD: What Changes}
Traditional CI/CD validates and deploys software artifacts. ML systems add data and model concerns:

\begin{itemize}
  \item data/schema validation;
  \item reproducible training and evaluation;
  \item model-quality release criteria;
  \item training-serving compatibility;
  \item staged deployment and rollback;
  \item post-release monitoring.
\end{itemize}

\begin{alertblock}{Continuous training is optional}
Retraining should be triggered by evidence and policy, not merely because new data exist. Automatic retraining without validation can propagate data-quality or labeling failures.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{ML CI/CD: Example Release Gate}
\begin{lstlisting}[style=code,basicstyle=\ttfamily\small]
jobs:
  validate:
    steps:
  ...
  evaluate:
    ...
    steps:
      ...
      - run: python check_release_gate.py
...
\begin{block}{Promotion}
Production promotion should depend on explicit release criteria and the deployment risk. High-impact systems may require manual approval, canary/shadow evidence, or additional governance checks.
\end{block}
\end{frame}
