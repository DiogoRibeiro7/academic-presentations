...
\begin{frame}[fragile,allowframebreaks]{The ML Production Gap}
\textbf{Why production ML is a systems problem}
...
\item \textbf{Reliability}: Defined service objectives, graceful degradation, and recoverable failure modes
\item \textbf{Scalability}: Handle growing load
\item \textbf{Governance}: Trace decisions, versions, approvals, and accountability-relevant artifacts
...
Deployment success depends on data contracts, reproducibility, monitoring, rollback, operational ownership, and evidence that the model remains fit for its intended use.
\end{alertblock}
\end{frame}

\begin{frame}[fragile,allowframebreaks]{The ML Lifecycle}
...
\begin{frame}[fragile,allowframebreaks]{Model Monitoring in Production}
