...
% Preserve Unicode mappings needed inside listings while inheriting the
% canonical visual code style.
\lstset{
    style=code,
    numbers=left,
...
\begin{itemize}
\item build robust feature pipelines across numerical, categorical, temporal, and domain-specific data;
\item choose and justify feature selection methods using statistical and model-based criteria;
\item prevent leakage and ensure reproducibility from experimentation to production deployment.
\end{itemize}
\end{frame}
...
Feature engineering is the construction of measurable representations from raw variables under domain, sampling, and deployment constraints. The objective is not to maximize the number of transformations, but to improve the information available to the downstream analysis without leaking unavailable information.
\end{block}
...
% Side annotation
\node[right=1cm] at (fe.east) {\textbf{Representation}};
\node[right=1cm] at (1.5,1.2) {\textbf{matters}};
\end{tikzpicture}
\end{figure}

\begin{alertblock}{Methodological point}
The amount of work spent on data preparation varies by problem. What matters statistically is that preprocessing decisions are learned only from information available at the relevant training or prediction time.
\end{alertblock}
\end{column}
\end{columns}
...
\item \textbf{StandardScaler}: $z = \frac{x - \mu}{\sigma}$
  \begin{itemize}
  \item Places features on a common variance scale; normality is not required
  \item Sensitive to outliers through the sample mean and standard deviation
  \end{itemize}
...
\begin{alertblock}{Scaling depends on the estimator}
Distance-based methods, penalized linear models, gradient-based optimization, and covariance-based methods can be scale-sensitive in different ways. Tree splits are usually invariant to monotone rescaling, but downstream preprocessing and regularization can still change the practical pipeline.
\end{alertblock}
\end{column}
\end{columns}
\end{frame}
...
Target encoding is supervised preprocessing. Encoding statistics must be estimated without using the validation/test target values; out-of-fold or ordered schemes are common approaches. Leave-one-out alone does not guarantee leakage-free model evaluation if the overall preprocessing is fitted before splitting.
\end{alertblock}
\end{column}
\end{columns}
\end{frame}
...
# Standardize when equal feature weighting on the variance scale is intended
...
methods = {
    ...
    'UMAP': 'Non-linear, fast, neighborhood-preserving embedding'
}
...
\item UMAP: Local/global geometry depends strongly on hyperparameters and preprocessing
\item All: Embeddings used for prediction must be fitted within the training/resampling pipeline
\end{itemize}
\end{alertblock}
\end{column}
\end{columns}
...
Feature selection changes the hypothesis space, regularization path, and data-dependent search procedure. Its effect on bias and variance depends on the estimator, signal structure, correlation among predictors, and how selection is validated.

\textbf{Goal:} improve the target criterion under a validation design that includes the selection procedure itself.
\end{column}
\begin{column}{0.5\textwidth}
\begin{figure}
\centering
...
\begin{frame}{Filter Methods: What They Measure}
\begin{itemize}
  \item \textbf{Variance threshold}: unsupervised removal of constant or near-constant predictors.
  \item \textbf{Univariate tests}: rank each predictor using a specified marginal relationship with the target.
  \item \textbf{Mutual information}: captures broader statistical dependence, subject to estimation error.
  \item \textbf{Correlation screening}: measures linear or monotone association depending on the coefficient.
\end{itemize}

\begin{alertblock}{Limitation}
Filter methods evaluate variables one at a time. They can miss interactions, redundancy, and model-specific usefulness.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Filter Selection Inside Cross-Validation}
Supervised filters belong inside the training fold:
...
\begin{block}{Why}
Selecting features once on the full dataset before cross-validation leaks target information into the validation folds and produces optimistic performance estimates.
\end{block}
\end{frame}

\begin{frame}{Wrapper Feature Selection}
Wrapper methods score candidate feature subsets using downstream model performance.

\begin{itemize}
  \item \textbf{Forward selection}: start small and add features greedily.
  \item \textbf{Backward elimination}: start with all features and remove them greedily.
  \item \textbf{RFE}: repeatedly fit a model and remove the least important features.
\end{itemize}

\begin{alertblock}{Cost}
Wrapper methods can require many model fits. Their computational cost depends on the search strategy, estimator, number of features, and validation scheme.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Wrapper Selection with Nested Evaluation}
Keep feature-subset search inside the training/resampling procedure:
...
Comparing many feature subsets on the same validation evidence can overfit the validation procedure. Nested resampling or an untouched final test set is appropriate when an unbiased performance estimate is required.
\end{block}
\end{frame}

\begin{frame}{Embedded Feature Selection}
Feature selection can be induced by the fitted model rather than performed as a separate filter step.

\begin{itemize}
  \item \textbf{Lasso}: an $L_1$ penalty can shrink some coefficients exactly to zero.
  \item \textbf{Elastic Net}: combines $L_1$ and $L_2$ penalties and can be more stable when predictors are correlated.
  \item \textbf{Tree-based models}: split criteria induce model-specific importance measures that can be used for selection.
\end{itemize}

\begin{alertblock}{Validation rule}
Hyperparameter tuning and feature selection are part of model fitting. If predictive performance is being estimated, the entire selection procedure must run inside each training fold.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Sparse Linear Selection}
Use cross-validation to tune the penalty, then inspect the fitted sparse model:
...
selected = np.flatnonzero(lasso.coef_ != 0)
\end{lstlisting}

\begin{block}{Interpretation}
A zero coefficient means the fitted penalized model excluded that predictor at the selected penalty. It does not prove that the variable is scientifically irrelevant or has no conditional association in another specification.
\end{block}
\end{frame}

\begin{frame}[fragile]{Tree-Based Embedded Selection}
Keep selection inside the resampling pipeline:
...
\begin{alertblock}{Importance is model-dependent}
Impurity-based tree importance can favor variables with many potential split points and can distribute importance unpredictably across correlated predictors. Treat the selected set as part of the fitted pipeline, not as a universal ranking of scientific relevance.
\end{alertblock}
\end{frame}
...
\begin{frame}{Pipeline Design and Reproducibility}
\begin{itemize}
  \item A pipeline treats preprocessing, feature selection, and the estimator as one fitted object.
  \item Data-dependent transformations are learned from the training partition only.
  \item The same fitted transformations are then applied to validation, test, and production data.
  \item Hyperparameter search can tune preprocessing and model parameters jointly without leaking validation information.
\end{itemize}

\begin{alertblock}{Reproducibility}
A pipeline does not make an analysis reproducible by itself. Also record data/version identifiers, code, random seeds, dependencies, and the evaluation protocol.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{Mixed-Type Feature Pipeline}
\begin{lstlisting}[style=code,basicstyle=\ttfamily\small]
...
Persist the fitted pipeline together with its code and dependency versions; monitor whether production inputs still satisfy the expected schema and feature semantics.
\end{block}
\end{frame}
...
\item[$\square$] Fit supervised selectors inside each training fold
\item[$\square$] Use model-based importance only on training/resampling data
\item[$\square$] Reserve untouched evaluation data for final assessment
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
\item[$\square$] Check whether each feature would exist at prediction time
\end{itemize}
...
\begin{frame}{Synthesis}
  \begin{itemize}
    \item Feature engineering is part of the statistical model: transformations change the information set, scale, geometry, and sometimes the estimand seen by the learner.
    \item Any transformation that uses the target or population-level statistics must be fitted inside the training portion of each resampling split when predictive performance is being estimated.
    \item Scaling is estimator-dependent. Standardization is not a normality requirement, and PCA on standardized variables answers a different question from PCA on the original covariance scale.
    \item Feature selection is itself a data-dependent search step. Selection outside cross-validation produces optimistic performance estimates and can distort post-selection inference.
    \item Dimensionality-reduction embeddings optimize method-specific objectives; attractive visual separation is not evidence of predictive usefulness or true cluster structure.
  \end{itemize}

  \begin{alertbox}{Pipeline principle}
    The reproducible unit is the entire transformation-and-model pipeline, evaluated under the same information constraints that will hold at prediction time.
  \end{alertbox}
\end{frame}
...
\begin{frame}{Selected References}
  \small
  \begin{itemize}
    \item M. Kuhn and K. Johnson (2019). \textit{Feature Engineering and Selection}. CRC Press.
    \item T. Hastie, R. Tibshirani, and J. Friedman (2009). \textit{The Elements of Statistical Learning}, 2nd ed. Springer.
    \item I. T. Jolliffe and J. Cadima (2016). "Principal component analysis: a review and recent developments." \textit{Philosophical Transactions of the Royal Society A}, 374.
    \item L. van der Maaten and G. Hinton (2008). "Visualizing Data using t-SNE." \textit{JMLR}, 9, 2579--2605.
    \item L. McInnes, J. Healy, and J. Melville (2018). "UMAP: Uniform Manifold Approximation and Projection for Dimension Reduction." arXiv:1802.03426.
  \end{itemize}
\end{frame}
