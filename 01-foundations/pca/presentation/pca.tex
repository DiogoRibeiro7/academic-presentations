...
\begin{frame}{Learning Goals}
  ...
  \begin{itemize}
    \item derive PCA from variance maximization, reconstruction error minimization, and the singular value decomposition;
    \item distinguish covariance-scale PCA from correlation-scale PCA and interpret principal directions only relative to the chosen centering/scaling convention;
    \item reason about subspace estimation error through eigengaps, principal angles, sample size, dimensionality, and distributional assumptions.
  \end{itemize}
\end{frame}
...
\begin{frame}{Explained Variance and Choosing $k$}
  \begin{itemize}
    \item For covariance PCA, total sample variance is $\mathrm{tr}(\Sigma_n)=\sum_{j=1}^d\lambda_j$, and the first $k$ components account for
      \[\mathrm{PVE}(k)=\frac{\sum_{j=1}^k\lambda_j}{\sum_{j=1}^d\lambda_j}.\]
    \item PVE is descriptive, not a universal model-selection criterion. A 90\% or 95\% threshold has no generic statistical justification.
    \item The relevant $k$ depends on the target: reconstruction, visualization, denoising, downstream prediction, or estimation of a latent subspace can imply different choices.
    \item If variables are standardized first, PCA is effectively performed on the correlation scale. This changes the geometry by giving variables equal marginal variance before decomposition.
    \item In high dimension, apparent eigenvalue separation can arise from sampling noise; stability across resamples and problem-specific spectral diagnostics can be more informative than a scree elbow alone.
  \end{itemize}
\end{frame}
...
\begin{frame}{Non-Asymptotic Excess Risk (High-Level Statement)} % ~5 min
  ...
  \begin{itemize}
    \item In the fixed-dimensional regular regime covered by the stated eigengap, moment, and concentration assumptions, the bound has order $1/n$.
    \item The leading constant encodes spectral separation and distributional information; the statement should not be extrapolated automatically to high-dimensional regimes where $d/n$ remains non-negligible.
  \end{itemize}
\end{frame}
...
\begin{frame}{Spiked Covariance Model} % ~4 min
  \begin{itemize}
    ...
    \item At the population level, when the non-zero signal eigenvalues are separated from the isotropic noise level, the leading eigenspace of $\Sigma$ coincides with the signal subspace.
  \end{itemize}
\end{frame}
...
\begin{frame}{Synthesis}
  \begin{itemize}
    \item PCA estimates a linear subspace, not a unique basis: rotations within the selected subspace represent the same geometric object.
    \item Variance maximization and reconstruction-error minimization are equivalent on centered data, but interpretation depends on whether PCA is performed on the covariance or correlation scale.
    \item Explained variance is a descriptive spectral quantity. Choosing the number of components should be tied to the inferential or predictive objective rather than a universal percentage threshold.
    \item Subspace estimation becomes unstable when relevant eigengaps are small; sampling variability, dimensionality, and higher-order moments influence finite-sample behavior.
    \item The $1/n$ excess-risk statements discussed here belong to a regular fixed-dimensional regime under explicit moment and eigengap assumptions; high-dimensional PCA can behave differently.
  \end{itemize}
\end{frame}
...
\begin{frame}{Selected References}
  \small
  \begin{itemize}
    \item I. T. Jolliffe (2002). \textit{Principal Component Analysis}, 2nd ed. Springer.
    ...
    \item T. Hastie, R. Tibshirani, and J. Friedman (2009). \textit{The Elements of Statistical Learning}, 2nd ed., Chapter 14.
    \item D. Paul (2007). "Asymptotics of sample eigenstructure for a large dimensional spiked covariance model." \textit{Statistica Sinica}, 17, 1617--1642.
    \item A. El Hanchi, M. A. Erdogdu, and C. J. Maddison. \textit{A Geometric Analysis of PCA}.
  \end{itemize}
\end{frame}
