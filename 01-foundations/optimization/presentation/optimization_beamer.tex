...
\begin{frame}{Learning Goals}
  ...
  \begin{itemize}
    \item distinguish convexity, smoothness, strong convexity, constraints, and stochasticity as separate properties that determine which guarantees are available;
    \item derive first- and second-order optimality conditions and state when KKT conditions are necessary or sufficient;
    \item choose and evaluate optimization algorithms by problem structure, oracle cost, conditioning, accuracy requirements, and computational budget rather than by a universal optimizer ranking.
  \end{itemize}
\end{frame}
...
\begin{frame}{Types of Optimization Problems}
  ...
  \begin{alertbox}{Computational complexity}
    Convexity gives powerful global optimality structure, but tractability still depends on the representation, oracle model, accuracy criterion, constraints, and problem class. Some non-convex or discrete problems are tractable, while many important formulations are computationally hard.
  \end{alertbox}
\end{frame}
...
\begin{frame}{Optimality Conditions}
  \begin{theorembox}{First-order necessary condition}
    If $f$ is differentiable and $\vect{x}^*$ is an unconstrained local minimum in the interior of the domain, then
    $$\nabla f(\vect{x}^*)=\vect{0}.$$
    Stationarity alone is not sufficient for a minimum in a general non-convex problem.
  \end{theorembox}

  \vspace{0.4em}

  \begin{theorembox}{Second-order conditions}
    At an interior local minimum of a twice-differentiable function, the Hessian is positive semidefinite. If $\nabla f(\vect{x}^*)=0$ and $\nabla^2 f(\vect{x}^*)\succ0$, then $\vect{x}^*$ is a strict local minimum.
  \end{theorembox}

  \vspace{0.4em}

  \begin{block}{Convex case}
    For differentiable convex $f$ on a convex domain, $\nabla f(\vect{x}^*)=0$ is sufficient for global optimality. For constrained convex problems the corresponding condition uses the normal cone or KKT conditions under appropriate regularity.
  \end{block}
\end{frame}
...
\begin{frame}{Gradient Descent}
  ...
  \textbf{One standard guarantee:}
  \begin{itemize}
    \item If $f$ is convex and $L$-smooth, fixed-step gradient descent with a suitable step size such as $\alpha\leq 1/L$ has a sublinear function-value guarantee of order $O(1/k)$.
    \item If $f$ is additionally strongly convex, stronger linear/geometric convergence bounds are available.
    \item For non-convex objectives, typical guarantees concern decrease, stationarity, or gradient norms rather than convergence to a global minimum.
  \end{itemize}
...
\begin{frame}{Adam Optimizer}
  ...
  \textbf{Why Adam is commonly used:}
  \begin{itemize}
    \item Per-coordinate scaling and momentum can make initial training less sensitive to heterogeneous gradient magnitudes.
    \item It is computationally inexpensive relative to second-order methods and works well in many large stochastic problems.
    \item Default hyperparameters are useful starting points, not guarantees; learning rate, schedule, batch size, parameterization, and objective geometry still matter.
  \end{itemize}
\end{frame}
...
\begin{frame}{Other Modern Optimizers}
  ...
  \begin{itemize}
    ...
    \item Separates multiplicative weight decay from the adaptive gradient update
    \item Often used when explicit weight decay is part of the training design
  \end{itemize}
...
\begin{frame}[fragile]{Gradient Descent Variants Comparison}
  ...
  \begin{alertbox}{Practical comparison}
    Optimizer choice should be treated as part of the experimental design. Compare plausible methods under matched training budgets and learning-rate schedules, and report both optimization performance and downstream generalization rather than assuming a universal default.
  \end{alertbox}
\end{frame}
...
\begin{frame}{KKT Conditions}
  \begin{theorembox}{Karush-Kuhn-Tucker (KKT) Conditions}
    Under a suitable constraint qualification, a local optimum of a differentiable constrained problem satisfies the KKT conditions. In a convex problem, KKT conditions are also sufficient under standard regularity such as Slater's condition:
...
\begin{frame}[fragile]{Training Loop: Optimization Step}
...
\begin{block}{Optimization state}
Gradient clipping, optimizer state, batch size, and learning rate all change the update dynamics. Treat them as part of the training algorithm.
\end{block}
\end{frame}

\begin{frame}[fragile]{Training Loop: Validation and Scheduling}
\begin{lstlisting}[style=code,basicstyle=\ttfamily\small]
scheduler.step()
...
\begin{alertblock}{Evaluation separation}
Validation guides model/optimizer selection. Final generalization performance should be estimated on data not reused for tuning, early stopping, or scheduler decisions.
\end{alertblock}
\end{frame}
...
\begin{frame}{Synthesis}
  \begin{itemize}
    \item Optimization guarantees are conditional on structure: convexity, smoothness, strong convexity, constraints, stochastic gradients, and oracle accuracy lead to different notions of convergence.
    \item Stationary points are not automatically minima in non-convex problems, and successful training of a predictive model is not the same as solving the population-risk problem globally.
    \item First-order, second-order, proximal, stochastic, derivative-free, Bayesian, and evolutionary methods exploit different information and cost models; no optimizer dominates across problem classes.
    \item Hyperparameter optimization is itself a statistical search problem. Validation noise, repeated selection, and computational budget should be included in the evaluation design.
    \item In machine learning, optimization error is only one component of performance; approximation, estimation, regularization, data quality, and distribution shift remain separate concerns.
  \end{itemize}
\end{frame}
...
\begin{frame}{Selected References}
  \small
  \begin{itemize}
    \item S. Boyd and L. Vandenberghe (2004). \textit{Convex Optimization}. Cambridge University Press.
    \item J. Nocedal and S. J. Wright (2006). \textit{Numerical Optimization}, 2nd ed. Springer.
    \item Y. Nesterov (2004). \textit{Introductory Lectures on Convex Optimization}. Springer.
    \item D. P. Kingma and J. Ba (2015). "Adam: A Method for Stochastic Optimization." \textit{ICLR}.
    \item I. Loshchilov and F. Hutter (2019). "Decoupled Weight Decay Regularization." \textit{ICLR}.
  \end{itemize}
\end{frame}
