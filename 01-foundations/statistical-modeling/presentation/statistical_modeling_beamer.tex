...
% Preserve the deck's listing layout while inheriting the canonical code style.
\lstset{
    style=code,
    numbers=left,
...
\begin{itemize}
\item explain bias-variance tradeoffs and generalization with rigorous learning-theory language;
\item use PAC/VC ideas to reason about model complexity and sample requirements;
\item choose validation and model-selection strategies that are statistically and operationally sound.
\end{itemize}
\end{frame}
...
\begin{frame}{Statistical Learning: Complementary Traditions}
  Statistical learning sits at the intersection of statistical inference, approximation, optimization, and algorithmic prediction.

  \begin{columns}
    \begin{column}[t]{0.5\textwidth}
      \textbf{Statistical emphasis}
      \begin{itemize}
        \item explicit stochastic models and estimands;
        \item sampling uncertainty and identifiability;
        \item diagnostics and interpretable parameters.
      \end{itemize}
    \end{column}
    \begin{column}[t]{0.5\textwidth}
      \textbf{Machine-learning emphasis}
      \begin{itemize}
        \item predictive risk and generalization;
        \item flexible function classes and optimization;
        \item high-dimensional and computational regimes.
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{alertblock}{The useful distinction}
    The methods overlap substantially. What changes is the target question, assumptions, loss function, and evidence required to justify a conclusion.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Scope of Generalization Guarantees}
  \begin{itemize}
    \item Classical PAC and VC results formalize generalization under a specified sampling framework, commonly i.i.d. observations from a fixed distribution.
    \item The guarantee is conditional on the hypothesis class, loss, confidence level, and assumptions used in the theorem; it is not a universal certificate for a deployed model.
    \item Dependence, adaptive data collection, distribution shift, label noise, or repeated model selection can require different arguments or invalidate a direct textbook bound.
    \item Bounds may be mathematically informative even when numerically loose; their value often lies in clarifying which quantities control learnability.
  \end{itemize}
\end{frame}
...
\vspace{0.3cm}
\textbf{Cross-validation:} a family of resampling procedures for estimating model-selection or predictive performance under a sampling scheme that should reflect the intended deployment population.
\end{frame}

\begin{frame}{Cross-Validation: Estimation Procedure}
\begin{enumerate}
  \item partition observations into $k$ folds;
  \item fit the complete modeling procedure on $k-1$ folds;
  \item evaluate the held-out fold using the target loss;
  \item repeat over folds and aggregate held-out losses.
\end{enumerate}

\begin{block}{What must stay inside each training fold}
Preprocessing, feature selection, hyperparameter tuning steps, and any data-dependent transformations whose fitted state uses the training sample.
\end{block}
\end{frame}

\begin{frame}[fragile]{Cross-Validation for Hyperparameter Selection}
...
There is no universal statement that CV variance simply decreases as $k$ increases, nor is LOOCV always nearly unbiased with high variance. Bias and variance depend on the estimator, loss, dependence structure, and data-generating process.
\end{alertblock}
\end{frame}
...
\begin{frame}{Emerging Frontiers: Distribution and Adaptation}
\begin{itemize}
  \item \textbf{Distribution shift}: theory for prediction when training and deployment distributions differ.
  \item \textbf{Domain adaptation / generalization}: transfer information across related environments under structural assumptions.
  \item \textbf{Continual learning}: update models over time without catastrophic loss of previously useful information.
  \item \textbf{Robust learning}: characterize performance under contamination, perturbation, or worst-case distributional neighborhoods.
\end{itemize}

\begin{alertblock}{Unifying issue}
Generalization guarantees are always relative to assumptions about how future data relate to the data used for training and selection.
\end{alertblock}
\end{frame}

\begin{frame}{Emerging Frontiers: Structure, Privacy, and Tasks}
\begin{itemize}
  \item \textbf{Causal learning}: combine predictive flexibility with invariance or identification assumptions about interventions.
  \item \textbf{Meta-learning}: exploit related tasks to accelerate adaptation, while controlling transfer and selection bias.
  \item \textbf{Federated learning}: optimize across distributed data holders under communication, heterogeneity, and privacy constraints.
  \item \textbf{Differential privacy}: quantify privacy loss through a formal randomized mechanism, usually at a cost in statistical efficiency.
\end{itemize}

\begin{block}{Theory evolves with the data-generating process}
Modern learning problems add adaptive collection, non-IID data, privacy mechanisms, and deployment feedback. Classical learning theory remains useful, but its assumptions must be updated rather than silently reused.
\end{block}
\end{frame}
...
\begin{frame}{Synthesis}
  \begin{itemize}
    \item Generalization is defined relative to a population, loss function, sampling mechanism, and hypothesis class; changing any of these changes the learning problem.
    \item Bias--variance decompositions, regularization, PAC/VC theory, stability, and cross-validation illuminate different aspects of model complexity and predictive risk rather than providing one universal complexity measure.
    \item Validation estimates are themselves statistical estimates. Their relevance depends on how closely the resampling design matches future data and how much model selection has been performed on the same evidence.
    \item Modern overparameterized models show that parameter count alone is an inadequate description of effective complexity; optimization dynamics, architecture, data geometry, and implicit regularization can matter.
    \item Predictive performance does not imply causal identification, robustness to distribution shift, fairness, or calibrated uncertainty. Those require additional assumptions and evaluation.
  \end{itemize}

  \begin{alertblock}{Practical principle}
    Use theory to make assumptions and failure modes explicit, then test the resulting claims against data collected in a way that represents the intended use.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Selected References}
  ...
  \begin{itemize}
    ...
    \item S. Shalev-Shwartz and S. Ben-David (2014). \textit{Understanding Machine Learning: From Theory to Algorithms}. Cambridge University Press.
    \item M. Mohri, A. Rostamizadeh, and A. Talwalkar (2018). \textit{Foundations of Machine Learning}, 2nd ed. MIT Press.
    \item V. N. Vapnik (1998). \textit{Statistical Learning Theory}. Wiley.
    \item M. Belkin, D. Hsu, S. Ma, and S. Mandal (2019). "Reconciling modern machine-learning practice and the classical bias--variance trade-off." \textit{PNAS}, 116(32), 15849--15854.
  \end{itemize}
\end{frame}
